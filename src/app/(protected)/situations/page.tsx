...
import { SituationsList } from "@/components/situations/SituationsList";
import { logProductEvent } from "@/lib/analytics/log-product-event";
import { PRODUCT_EVENTS } from "@/lib/analytics/product-events";
import { loadSituationsList } from "@/lib/situations/load-situations-list";
...
export default async function SituationsPage() {
  ...
  if (!user) {
    redirect("/login?next=/situations");
  }

  const snapshot = await loadSituationsList(supabase, user.id);

  await logProductEvent(supabase, {
    userId: user.id,
    eventType: PRODUCT_EVENTS.SITUATION_OPENED,
    payload: { view: "list", count: snapshot.situations.length },
  });
  ...
  return (
    <div className="flex min-h-full flex-1 flex-col bg-[#0F172A]">
        ...
        <div className="mx-auto w-full max-w-[480px]">
          ...
          <div className="rounded-2xl border border-white/8 bg-[#162033] px-5 py-5">
            ...
            <p className="mt-2 max-w-[40ch] text-sm leading-relaxed text-white/60">
              Your debt cases, offers, deadlines, and next steps — organized in one place.
            </p>
            {snapshot.recoveryStageLabel ? (
              <p className="mt-3 text-xs text-white/40">
                Recovery stage · {snapshot.recoveryStageLabel}
              </p>
            ) : null}
          </div>
        </div>
      ...
      <main className="mx-auto w-full max-w-[480px] flex-1 px-4 py-8 sm:px-6">
        <SituationsList snapshot={snapshot} />
      </main>
    </div>
  );
}
